...
describe(`Consideration (version: ${VERSION}) — initial test suite`, function () {
  ...
  const createTransferHelperItemWithApproval = async (
    contract,
    receiver,
    itemType,
    approvalAddress
    ...
    switch (itemType) {
      ...
      case 4: // ERC721_WITH_CRITERIA
        amount = 1;
        tokenIdentifier = randomBN();
        await contract.mint(receiver.address, tokenIdentifier);
      ...
      case 3: // ERC1155
      case 5: // ERC1155_WITH_CRITERIA
        tokenIdentifier = random128();
        amount = minRandom(1);
        await contract.mint(receiver.address, tokenIdentifier, amount);

        // Receiver approves contract to transfer tokens
        await set1155ApprovalForAll(receiver, approvalAddress, true, contract);
        break;
    }
    return { itemType, token, tokenIdentifier, amount };
  };
